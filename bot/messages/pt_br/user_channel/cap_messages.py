...
def cap_command_capture_messages (dados): 
    _item = "" if dados["dados_horda"]["dados_aventureiro"]["item"] == "" else f"""usou o item {dados["dados_horda"]["dados_aventureiro"]['item']} e """    
    ...
    _pronta_evoluir = "" if  _criatura_ja_capturada["cp"] + dados["dados_horda"]["capturado"]["cp"] < dados["dados_horda"]["criatura"]["cp_limite"] else "A criatura está pronta pra evoluir, você pode obter o item capevol ou capevol+ no capshop"  
...
error_horde_inactive = lambda dados : f'{dados["aventureiro"]}, não há {tipo_horda(dados["nome_horda"])} ativa no momento!'

def error_adventure_already_participated (dados): 
    has_captured = ", mas não conseguiu capturar a criatura" if not dados["dados_aventureiro"]["capturou"] else " e conseguiu capturar a criatura"
...
def error_invalid_attempties(dados): 
    opcoes = "" if dados["nome_horda"] != "elemental" else f"""Opções: {','.join([c["nome"] for c in dados["criaturas"]])}""" 
...
error_invalid_creature = lambda dados : f"""{dados["aventureiro"]}, criatura inválida, Opções: {', '.join(dados["opcoes"])}"""

error_overload_attempties = lambda dados : f"""{dados["aventureiro"]}, o custo total excede a quantidade de capcoins que você possui. Custo da tentativa: {dados["custo"]} Capcoins atuais: {dados["capcoins"]}"""